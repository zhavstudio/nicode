...
class GenerateReactAdminPanelRoutes extends Command
{
    ...
    public function handle()
    {
        ...
        $routes = $allRoutes->filter(function ($item) {
            return (in_array("api", explode(".", $item->getName()), true) && in_array("admin", explode(".", $item->getName()), true))  || $item->getName() === "logout" || $item->getName() === "login"|| $item->getName() === "api.public.temporary-file.store";
        })->map(function ($route) {
            return [
                'uri' => $route->uri(),
                'name' => $route->getName(),
...
//        $routes->toJson(JSON_OBJECT_AS_ARRAY);
        ...
        Storage::disk("resource")->put('js/views/admin-panel/routes.json', $objected);
